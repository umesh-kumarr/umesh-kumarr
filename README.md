# 👋 Hi, I'm Umesh Kumar

**Data Analyst | Machine Learning Engineer | Open to Remote Work**

📍 **Location:** Gurugram, India | 🌍 **Interested in:** Digital Nomad Opportunities  
📧 **Email:** umeshkumarr.ds@gmail.com | 💼 **LinkedIn:** [linkedin.com/in/umeshkumar02](https://www.linkedin.com/in/umeshkumar02/)
🔗 **Website:** Coming soon

---

## 🎯 About Me

I'm a passionate data analyst with 6+ months of hands-on experience building machine learning models and designing data-driven solutions. I recently completed my **BTech in Data Science** and am now focusing on developing my freelance data analysis business while building my digital nomad career.

**What I Do:**
- 🤖 Build predictive ML models (Classification, Regression, Time Series)
- 📊 Design interactive dashboards (Tableau, Power BI)
- 📈 Perform statistical analysis and hypothesis testing
- 🔧 Optimize SQL queries for performance
- 🎨 Create compelling data visualizations
- 📋 Deliver end-to-end data solutions

**Why Work With Me:**
- ✅ Fast turnaround (deliver 2 days early)
- ✅ Daily communication & proactive updates
- ✅ High-quality work (95%+ client satisfaction)
- ✅ Flexible for calls & discussions
- ✅ Production-ready code with documentation

---

## 🛠️ Technical Skills

**Languages:** Python | SQL | R (basics)

**Data Science & ML:** Pandas | NumPy | Scikit-learn | XGBoost | Matplotlib | Seaborn | Statistics | Feature Engineering

**Visualization:** Tableau | Power BI | Matplotlib | Seaborn

**Databases:** PostgreSQL | MySQL | SQLite

**Tools & Platforms:** Git/GitHub | Jupyter Notebook | VSCode | Excel (Advanced) | Anaconda

**ML Concepts:** Regression | Classification | Time Series Forecasting | Feature Engineering | Model Evaluation | Cross-Validation

---

## 📊 Featured Projects

*Projects coming soon! Check back for:*
- Customer Churn Prediction Model
- Sales Forecasting with Prophet
- Interactive Tableau Dashboard
- NLP Text Analysis
- Credit Risk Modeling

(These will be added as they're completed)

---

## 📈 Current Learning Goals (Next 90 Days)

- ✅ Master Advanced SQL (Window Functions, CTEs)
- ✅ Build 5 production-ready portfolio projects
- ✅ Achieve ₹30,000-50,000/month freelance income
- ✅ Grow personal brand (LinkedIn, Twitter, Blog)
- ✅ Obtain cloud certification (AWS/GCP)
- ✅ Secure digital nomad visa

---

## 📚 Open to Opportunities

I'm actively looking for:
- **Freelance Data Analysis Projects** (Fixed-price, Hourly, Retainer)
- **Contract Data Analyst Roles** (Remote, 3-12 months)
- **Part-time Remote Positions** (Flexible schedule)
- **Consulting & Advisory Work** (Data strategy, analytics)

---

## 📈 GitHub Stats

![Your GitHub Stats](https://github-readme-stats.vercel.app/api?username=umesh-kumarr&show_icons=true&theme=radical)

---

## 🤝 Let's Connect

📧 **Email:** umeshkumarr.ds@gmail.com  
💼 **LinkedIn:** linkedin.com/in/umeshkumar02 
💬 **Message me:** Always open to chat about data projects!

---

*Last Updated: January 06, 2026*  
*Actively seeking freelance clients and remote opportunities*  
*📍 Building my digital nomad career, one project at a time*
